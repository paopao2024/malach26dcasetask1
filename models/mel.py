...
class AugmentMelSTFT(nn.Module):
    ...
    def forward(self, x):
        # shape: batch size x samples (12 x 32,000)
        ...
        mel_basis = torchaudio.functional.melscale_fbanks(self.n_fft // 2 + 1, fmin, fmax, self.n_mels, self.sr)
        mel_basis = torch.as_tensor(mel_basis.T, device=x.device)
