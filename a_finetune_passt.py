...
W&B logs only the metrics that matter: train/loss, val/loss, val/hP, val/hR,
val/hF (OFFICIAL DCASE metric, verbatim from evaluate.py). A config block
prints at startup.
...
DEFAULT_OUTPUT_ROOT     = Path.home() / "malach26dcasetask1" / "outputs" / "a_passt_ft_official"
...
class PaSSTClassifier(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        self.log("val/hP", _safe(hP), on_step=False, on_epoch=True)
        self.log("val/hR", _safe(hR), on_step=False, on_epoch=True)
        self.log("val/hF", _safe(hF), on_step=False, on_epoch=True, prog_bar=True)
    ...
    def on_test_epoch_end(self):
        ...
        self.log("test/hP", _safe(hP))
        self.log("test/hR", _safe(hR))
        self.log("test/hF", _safe(hF))
...
def main():
    ...
    run_id = (
        ...
        f"_a_passt_ft_official_fold{args.fold}"
