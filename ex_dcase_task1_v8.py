...
class Classifier(pl.LightningModule):
    def __init__(self, lr=1e-3, encoder_lr=1e-5, gamma=2.0,
                 label_smoothing=0.1, unfreeze_blocks=2, verbose=False):
        super().__init__()
        self.lr = lr
        ...
        # --- load CLAP and freeze everything ---
        ...
        # --- unfreeze the last N HTSAT transformer blocks + audio projection ---
        self._unfreeze_top(self.unfreeze_blocks)

        self.head = MLP()

    def _unfreeze_top(self, n_blocks):
        """Unfreeze the last n transformer blocks of HTSAT and the audio projection.
        Names are matched defensively because laion_clap nests the HTSAT
        under self.clap.model.audio_branch."""
        unfrozen = []
        named = list(self.clap.named_parameters())
        # collect transformer block indices present in the audio branch
        block_ids = set()
        for name, _ in named:
            # typical path: model.audio_branch.layers.{i}.blocks.{j}...
            if "audio_branch" in name and ".blocks." in name:
                try:
                    seg = name.split(".blocks.")[1]
                    block_ids.add(int(seg.split(".")[0]))
                except (IndexError, ValueError):
                    pass
        keep = sorted(block_ids)[-n_blocks:] if block_ids else []
        for name, p in named:
            is_top_block = any(f".blocks.{b}." in name for b in keep) and "audio_branch" in name
            is_audio_proj = "audio_projection" in name
            is_norm = "audio_branch" in name and name.endswith("norm.weight") or name.endswith("norm.bias")
            if is_top_block or is_audio_proj:
                p.requires_grad = True
                unfrozen.append(name)
        print(f"  unfroze {len(unfrozen)} encoder tensors "
              f"(top {n_blocks} HTSAT blocks {keep} + audio projection)")
    ...
    def forward(self, wav, text):
        audio = self.encode_audio(wav)          # live, partially trainable
        x = torch.cat([audio, text], dim=1)     # 512 + 512
        return self.head(x)

    def configure_optimizers(self):
        enc_params  = [p for p in self.clap.parameters() if p.requires_grad]
...
def run(dataset_path, n_epochs=15, batch_size=16, seed=42,
        early_stop_patience=5, gamma=2.0, label_smoothing=0.1,
        lr=1e-3, encoder_lr=1e-5, unfreeze_blocks=2,
        run_tag="v8", verbose=False):
    ...
    print(f"config: FINE-TUNE CLAP audio (top {unfreeze_blocks} blocks) + frozen text")
    print(f"        head_lr={lr}, encoder_lr={encoder_lr}, batch={batch_size}, "
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, df["class"]), 1):
        ...
        trainer = pl.Trainer(
            ...
            accelerator="auto",
            devices=1,
            precision="16-mixed",          # fp16 to fit HTSAT in 12 GB
        ...
        model = Classifier(lr=lr, encoder_lr=encoder_lr, gamma=gamma,
                           ...
                           unfreeze_blocks=unfreeze_blocks, verbose=verbose)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", type=str, default="v8")
    ...
    run(args.dataset_path, n_epochs=args.epochs, batch_size=args.batch_size,
        ...
        encoder_lr=args.encoder_lr, unfreeze_blocks=args.unfreeze_blocks,
