"""
DCASE 2026 Task 1 - v5
Real ensemble: train multiple classifiers, average their predicted probabilities.

This is different from v4's feature concatenation. Each classifier is trained
independently on its own feature set, then we average their softmax outputs
to get final predictions. This sidesteps the feature-alignment problem.

Default ensembles `clap` + `passt`. You can pass any combination:
  --models clap passt
  --models clap passt passt+clap_text
  --models clap passt passt+clap passt+clap_text   # everything
"""
...
FEATURE_CONFIGS = {
    ...
    "passt+clap":      {"dirs": ["passt_embeddings", "clap_audio_embeddings", "clap_text_embeddings"], "dim": 1792},
}
...
def hierarchy_aware_from_probs(probs):
    """Apply hierarchy adjustment to already-softmaxed probabilities."""
...
# ── train one model, return its predictions on val set ──────────────────
def train_and_predict(features, train_df, val_df, dataset_path,
    ...
    # single GPU keeps inference clean (no DDP gather needed afterwards)
    trainer = pl.Trainer(
        max_epochs=n_epochs,
        logger=logger,
        accelerator="auto",
    ...
    # run inference on val set to get probabilities
...
# ── main: orchestrate the ensemble across folds ─────────────────────────
def run_ensemble(dataset_path, feature_sets, n_epochs=50, patience=10,
                 batch_size=256, seed=42, run_tag="v5"):
    ...
    # filter df to sounds that have all features needed by any model
    all_dirs = set()
    for fs in feature_sets:
    ...
    print(f"\n{'='*70}")
    print(f"baseline (v1)  : 0.8013")
    print(f"v3 default     : 0.8005")
    print(f"v4 passt+ctext : 0.7629")
    print(f"v4 passt alone : 0.7403")
    print(f"{'='*70}\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--models", type=str, nargs="+", default=["clap", "passt"],
